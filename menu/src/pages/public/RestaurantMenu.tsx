...
export const RestaurantMenu: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribeTenant = onSnapshot(tenantQ, (snapshot) => {
      if (snapshot.empty) { setError(true); setLoading(false); return; }
      setTenant({ id: snapshot.docs[0].id, ...snapshot.docs[0].data() } as Tenant);
    }, (err) => { console.error(err); setError(true); setLoading(false); });
  ...
  const hasPromo = products.some((p: Product) => p.promotionalPrice && p.promotionalPrice > 0);
  ...
  const filteredProducts: Product[] = (() => {
    if (selectedCategoryId === '__promo__') {
      return products.filter((p: Product) => p.promotionalPrice && p.promotionalPrice > 0);
    }
    if (selectedCategoryId) {
      return sortWithPromoFirst(products.filter((p: Product) => p.categoryId === selectedCategoryId));
    }
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans flex items-center justify-center">
      <div className="w-full max-w-lg bg-white min-h-screen shadow-xl relative flex flex-col overflow-hidden">
        ...
        {/* Header */}
        <header className="pt-8 pb-12 px-6 text-white relative z-10 shrink-0" style={{ backgroundColor: themeColor }}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              {tenant.logoUrl && (
                <div className="w-10 h-10 rounded-full bg-white overflow-hidden shadow-sm shrink-0">
        ...
        {/* Menu List */}
        <main className={`${showFilterBar ? 'mt-3' : '-mt-6'} flex-1 px-4 relative z-20 overflow-y-auto pb-24`}>
          {tenant.isOpen === false && (
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 bg-white border-t border-slate-100 z-30 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="bg-white/20 px-2 py-0.5 rounded text-[10px]">{itemCount}</span>
              </div>
        ...
        <CartModal isOpen={isCartOpen} onClose={() => setIsCartOpen(false)} tenant={tenant} />
      </div>
    </div>
  );
};
